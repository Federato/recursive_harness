...
## ~~D1 · A state that has no value never falls back to the national one~~ — **OI-88** ✅ **CLOSED 2026-08-17**
...
**Closed 2026-08-17, and it did unblock more than the one case.** **Size-of-risk now rates in 51 of
51 jurisdictions, up from 2**, and Oklahoma lands on **8816** — ISO's own figure — matching the live
service on the premium and every published field.

**It was measured before it was decided,** which changed the fix. Under 1% of `FirstNonNull` sites
are affected (**69 realistically, in 29 of 570 packages**), so the change is small — but **51 of
those 69 have a trailing `Constant` standing ready to answer.** At three sites in four, loosening
the refusal carelessly would have turned a genuinely missing value into a plausible premium with
nothing recorded. So the blunt fix was rejected, the narrow one taken — **only the null-in-arithmetic
refusal is absorbed, and only inside a `FirstNonNull` branch** — and **the abandonment is traced**,
so a masked null stays auditable. The safety property has its own test: a branch that fails for any
*other* reason must still refuse.

**One thing it revealed.** CA and NY rate but come back **unchanged from base** — their state rule
sets override countrywide wholesale (N3). The refusal had been hiding that. It belongs to D3/OI-89's
territory, not this one.

## ~~D2 · Two counts of how a jurisdiction locates a risk, and they do not reconcile~~ — **OI-91** ✅ **CLOSED 2026-08-17**
...
**Closed 2026-08-17, and the answer was that they never disagreed.** `4 + 11 = 15`. The four and the
eleven are **one population of fifteen**, not two camps plus a remainder — all fifteen declare the
**same field**, `GeneralLiabilityLocation.TerrorismTerritory`, and the split is only which domain
table backs it. The second measurement's fifteen is the same fifteen, jurisdiction for jurisdiction.
Nobody had lined them up, so a subdivision read as a contradiction.

**And the blocking was ours.** Countrywide reads no terrorism location at all, so the 36 that file
none are not missing an input — there is nothing to send. The `ZipCode` fallback that produced the
16/20 split was **inert**, rating identically with and without in six jurisdictions.

**Terrorism now rates in 51 of 51, up from 31**, and the four that had been blocked agree with ISO
live: **AK 7415, VT 7400, WY 8501, MT 7908 — all `MATCH` on the premium and every published field.**

**It raised one new item, OI-93.** NY rates terrorism unchanged from base and ISO agrees, because NY
territory `001` genuinely carries no charge while `002`–`006` charge 110. The variant generator takes
`values()[0]`, so on NY the variant exercises nothing while reporting as rated.
...
## ~~2. Close the rounding question~~ — **OI-70** ✅ **CLOSED 2026-08-17**
...
**Closed 2026-08-17. ISO rounds half-up.** The med-pay route was not needed — `exposure / 1000` is
itself rounded at 0dp, so an exposure of the form **1,500,000** puts a rating product on exactly
`2164.5`, `x.5` with `x` even. **Four ties built, four rated live, four agreements: 2469, 4116, 5761,
9054** — where half-even predicts 2467, 4115, 5760, 9052. **The engine's default was right and is now
evidenced rather than assumed.**

**A second hypothesis was raised and tested rather than argued:** that ISO truncates to four digits
and then rounds. **It changes 0 of 432 rounding operations**, including the 30 whose input has more
than four decimals — the `.5` threshold at 0dp and 3dp is exactly representable at 4dp, so truncating
