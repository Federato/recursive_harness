...
|---|---|---|
| **Rules** | `text/rules/` — 503 notices, 502 readable, 51 jurisdictions, 2021–2027 | Rules 1–56, state exception pages, ILF tables (56.B), ILTAs (56.C), Classification Table, **Territory Pages (`CG-T-n`)** |
| **Loss Costs** | `text/losscosts/` — 472 notices, 471 readable, 51 jurisdictions, 2020–2027 | Published loss costs by class/territory/subline, ELP Supplement (Procedures 1–5, Tables 5.B–5.E), loss cost addendum mappings |
| **Terrorism** *(added 2026-08-12)* | `text/terrorism/` — 3 notices, 113–118 pages each, multistate, 2020–2022 | The **Terrorism Supplement** to the CLM. Rule 55 Terrorism Endorsement Options (**8 `TEV` versions**) and Terrorism Premium Determination (**16 `PEV` versions**), the state-to-version assignment table, and the "above average" exposure classification list |
| **Schedule & Experience** *(added 2026-08-12)* | `text/scheduleexperience/` — 52 notices (`CGLES`), 192 pages, 2023–2024 | The **Commercial General Liability Experience And Schedule Rating Plan**. 16 rules: eligibility, the experience modification `((AER − EER) ÷ EER) × Credibility`, **Rule 9 Table 9** schedule-rating characteristics, and **Rule 16** credibility / expected experience ratio / maximum single loss bands. One document per jurisdiction plus `MU` |
| **Composite Rating** *(added 2026-08-12)* | `text/compositerating/` — 90 notices (`CRP`), 462 pages, 2007–2024 | The **Composite Rating Plan**. **Two families: `GL-` (39, 2007–2012) and `IL-` (51, 2017+)** — it moved to the *Interline* manual in 2017, so a filename sweep for `GL-*` finds 39 of 90 |

Text is page-tagged (`<<<PAGE n>>>`), extracted with `pypdf` — the extractor that preserves
grid row pairing. Source PDFs are at `../../Commercial Line Manuals/GL/{Rules,LossCosts,Terrorism}/`.

**The corpus is 1,122 text files against 1,122 documents on disk** (503 rules + 472 loss costs +
3 terrorism + 54 schedule & experience + 90 composite), and `notices.json` registers **1,121** —
the two truncated source PDFs
(`GL-MO-2027-RU-003`, `GL-MI-2027-LC-003`) have 0-byte text placeholders and are registered
inconsistently, one yes and one no. **Neither can be quoted; both have a usable prior notice.**
OI-56. **Everything on disk is now ingested** — the last two families landed 2026-08-12 and closed OI-55.
When a question falls outside these five corpora, say which corpus would hold the answer rather
than reporting silence: silence from a corpus you do not hold is not evidence.

| Knowledge file | Contents |
|---|---|
| `knowledge/invariants.json` | **32 verified invariants** (17 BLOCKER, 10 MAJOR, 5 MINOR) with evidence and check. Your review checklist. |
| `knowledge/jurisdictions.json` | Per-state: latest notices, territory scheme + domain, rate vintage, payroll shape, liquor grade, ILF inventory, deviation map |
| `knowledge/circulars.json` | 727 circulars → description, type, filings, ERC editions, states |
| `knowledge/notices.json` | All 1,119 notices → circulars, filings, ERC edition, effective date, **date confidence**. Five groups: `rules`, `losscosts`, `terrorism`, `scheduleexperience`, `compositerating`. **The two plan families carry `date_confidence: Low`** — they cite no circular, so their dates are the notice year |
| `knowledge/terrorism.json` | The Terrorism Supplement, decoded: **52 jurisdiction→version assignments**, 24 version blocks with page spans, the **142 above-average class codes**, and the four filed factors with citations |
...
`tools/iso.py` — run it. Do not hand-grep 975 files when a subcommand exists.

```bash
python tools/iso.py state NJ                        # jurisdiction profile
python tools/iso.py territory NJ --zip 07030        # -> HOBOKEN, territory 504, notice+page
python tools/iso.py rate TX --class 10010           # loss cost per territory, with meaning
python tools/iso.py rule 45 --st TX                 # rule text in a jurisdiction's notice
python tools/iso.py rule 56 --st MU                 # countrywide base
python tools/iso.py grep "displayed in the state exceptions" --kind RU
python tools/iso.py grep "OWNERSANDCONTRACTORS" --squash    # pypdf injects spaces: use --squash
python tools/iso.py page GL-NJ-2026-RU-001 27       # full page text
python tools/iso.py circular LI-GL-2022-325         # circular -> editions, filings, states
python tools/iso.py notice GL-NJ-2026-RU-001        # notice metadata + date confidence
python tools/iso.py effective NJ --date 2026-06-01  # which notices governed at a date
python tools/iso.py invariant --severity BLOCKER    # the checklist
python tools/iso.py grep "0.58" --kind TER          # terrorism supplement only
python tools/iso.py notice GL-MU-2022-TERXV-001     # terrorism edition metadata
python tools/iso.py grep "credibility" --kind SE    # schedule & experience plan
python tools/iso.py grep "composite rate" --kind CR # composite rating plan
python tools/iso.py grep "..." --kind PLANS         # both rating plans at once
```
